...
class DataPersistence:
    def __init__(self):
        ...
        self._current_count = 0  # in-memory count for accurate real-time tracking
        self._connect()
        self._init_db()
    ...
    def _init_db(self):
        with self._conn.cursor() as cur:
            ...
            # NEW: Realtime metrics table — written every 1s, read by Grafana
            cur.execute(f"""
    ...
    def _load_or_create_session(self):
        ...
        if row and row["starting_count"] == STARTING_COUNT:
            ...
            self._current_count = row["current_count"]  # load count from DB into memory
            ...
            self._current_count = STARTING_COUNT  # initialize in-memory count
            self._conn.commit()
    ...
    def _log_events(self, event_type, delta, count):
        current = self._current_count  # use in-memory count instead of querying DB
    ...
    def add_entries(self, count):
        self._current_count += count  # update memory first for accurate real-time count
    ...
    def get_current_count(self):
        return self._current_count  # read from memory, not DB
    ...
    # NEW: Realtime metrics for Grafana
    ...
    def update_realtime_metrics(self, roi_count, occupancy, fps, wifi_signal):
        ...
        try:
            with self._conn.cursor() as cur:
                ...
                # Keep only last 24h of data to avoid unbounded growth
                cur.execute(f"""
