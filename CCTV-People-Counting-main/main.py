...
from web_stream import start_server, update_frame   # ← NEW
from wifi_signal import get_wifi_signal_percent     # ← NEW
...
def main():
    ...
    has_display = False
    if has_display:
        print("✓ Display detected — video window will be shown")
    else:
        print("⚠ No display detected — running headless (logs only)")
    ...
    line_tracker = CentroidLineCrossingTracker(
        line_y=line2_enter_y,   # pakai salah satu saja, enter_y atau exit_y
        max_disappeared=10
    )
    ...
    # ── Start web dashboard ────────────────────────────────────────────────────
    start_server(port=8080)  # ← NEW: accessible at http://<jetson-ip>:8080
    _last_metrics_write = 0  # ← NEW: throttle DB metrics writes to every 1s
    ...
    try:
        while True:
            ...
            # ── Web stream + metrics ───────────────────────────────────────────
            update_frame(combined_view)  # ← NEW: send frame to browser

            _now = time.time()           # ← NEW: write metrics every 1s to DB
            if _now - _last_metrics_write >= 1.0:
                data_persistence.update_realtime_metrics(
                    roi_count   = count_v1,
                    occupancy   = count_v2,
                    fps         = fps_process,
                    wifi_signal = get_wifi_signal_percent()
                )
                _last_metrics_write = _now
            # ── End web stream + metrics ───────────────────────────────────────
